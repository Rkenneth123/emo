...
export default function BehaviorLevel3({ navigation }) {
  ...
  const handleNextLevel = async () => {
    if (score >= 3) {
      navigation.navigate('BehaviorEmotionLevel4');
    } else {
  ...
  const optionClicked = async (isCorrect) => {
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      await AsyncStorage.setItem('BehaviorLevel3', (score + 1).toString());
    }
  };
  const load = async () => {
        ...
        {showFinalResults ? (
          <View style={styles.finalResultMainContainer}>
            <View style={styles.finalResultContainer}>
              {score >= 3 ? (
                <View>
                  <Text>New Achievement Unlock – Behavior Test Level 4</Text>
                  <Image
