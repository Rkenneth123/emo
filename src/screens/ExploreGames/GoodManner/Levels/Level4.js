...
export default function GoodEmotionLevel4({ navigation }) {
  ...
  const optionClicked = async (isCorrect) => {
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      await AsyncStorage.setItem('GoodEmotionLevel4', (score + 1).toString());
    }
  };
  const load = async () => {
        ...
        {showFinalResults ? (
          <View style={styles.finalResultMainContainer}>
            <View style={styles.finalResultContainer}>
              {score >= 3 ? (
                <View>
                  <Text>New Achievement Unlock – Good Manners Level 5</Text>
                  <Image
