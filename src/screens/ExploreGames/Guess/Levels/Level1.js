...
export default function GuessLevel1({ navigation }) {
  ...
  const optionClicked = async (isCorrect) => {
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      await AsyncStorage.setItem('GuessLevel1', (score + 1).toString());
    }
  };
  const load = async () => {
        ...
        {showFinalResults ? (
          <View style={styles.finalResultMainContainer}>
            <View style={styles.finalResultContainer}>
              {score >= 3 ? (
                <View>
                  <Text>New Achievement Unlock – Guess Emotion Level 2</Text>
                  <Image
